...
export default function DraggableColumnHeader({
  ...
  return (
    <TableCell ref={dropRef as unknown as React.Ref<any>} colSpan={header.colSpan} sx={{ cursor: 'move' }} {...header.column.columnDef.meta}>
      <Box component="span" ref={previewRef as unknown as React.Ref<any>}>
        <Box ref={dragRef as unknown as React.Ref<any>} sx={{ color: isOverCurrent ? 'primary.main' : 'text.primary', opacity: isDragging ? 0.9 : 1 }}>
          {children}
        </Box>
      </Box>
    </TableCell>
