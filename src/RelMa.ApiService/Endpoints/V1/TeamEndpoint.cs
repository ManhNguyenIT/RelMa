...
using RelMa.Application.UseCases.Teams.V1.Commands;
...
internal sealed class TeamEndpoint : IEndpoint
{
    private const string BaseUrl = "/api/v{version:apiVersion}/teams";
    ...
    public static async Task<PagedResult<TeamResponse>?> List(
        IMediator mediator,
    ...
    public static async Task<bool> Delete(
        ...
        IConnectionMultiplexer multiplexer,
        IOptions<RedisCacheOptions> options,
        [AsParameters] DeleteTeamCommand command,
        CancellationToken cancellationToken)
    {
