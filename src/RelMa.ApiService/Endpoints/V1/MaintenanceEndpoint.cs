...
using RelMa.Application.UseCases.Maintenances.V1.Commands;
using RelMa.Application.UseCases.Maintenances.V1.Queries;
...
internal sealed class MaintenanceEndpoint : IEndpoint
{
    private const string BaseUrl = "/api/v{version:apiVersion}/maintenances";
    ...
    public static async Task<PagedResult<MaintenanceResponse>?> List(
        IMediator mediator,
        IDistributedCache cache,
        IUserContext userContext,
        [AsParameters] GetMaintenanceQuery query,
        CancellationToken cancellationToken)
        => await cache.GetOrCreateAsync(
            key: $"{userContext.TenantId}:maintenances",
            param: query,
            factory: async token => await mediator.SendQueryAsync<GetMaintenanceQuery, PagedResult<MaintenanceResponse>>(query, token),
    ...
    public static async Task<DefaultIdType> Create(
        ...
        CancellationToken cancellationToken)
    {
        var result = await mediator.SendCommandAsync<CreateMaintenanceCommand, DefaultIdType>(command, cancellationToken);
        string[] patterns =
        [
            $"{userContext.TenantId}:maintenances",
        ];
    ...
    public static async Task<DefaultIdType> Update(
        ...
        CancellationToken cancellationToken)
    {
        var result = await mediator.SendCommandAsync<UpdateMaintenanceCommand, DefaultIdType>(command, cancellationToken);
        string[] patterns =
        [
            $"{userContext.TenantId}:maintenances",
        ];
    ...
    public static async Task<bool> Delete(
        ...
        IConnectionMultiplexer multiplexer,
        IOptions<RedisCacheOptions> options,
        [AsParameters] DeleteMaintenanceCommand command,
        CancellationToken cancellationToken)
    {
        var result = await mediator.SendCommandAsync<DeleteMaintenanceCommand, bool>(command, cancellationToken);
        string[] patterns =
        [
            $"{userContext.TenantId}:maintenances",
        ];
    ...
    public static async Task<IResult> Export()
    {
        ...
        return Results.File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "maintenances_exported.xlsx");
    }
    ...
    public static async Task<IResult> Template()
    {
        await Task.CompletedTask;
        using var stream = new MemoryStream();
        return Results.File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "maintenances_template.xlsx");
    }
}
